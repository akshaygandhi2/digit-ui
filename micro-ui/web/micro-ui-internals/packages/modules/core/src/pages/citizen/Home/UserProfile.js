...
const UserProfile = ({ stateCode, userType, cityDetails }) => {
  ...
  const setUserMobileNumber = (value) => {
    ...
    if (userType === "employee" && !new RegExp(/^77[0-9]{6}$/).test(value)) {
  ...
  const updateProfile = async () => {
    ...
    try {
      ...
      if (userType === "employee" && !new RegExp(/^77[0-9]{6}$/).test(mobileNumber)) {
              ...
              <LabelFieldPair style={{ display: "flex" }}>
                ...
                <div style={{ width: "100%" }}>
                  <MobileNumber
                    ...
                    disable={true}
                    {...{ required: true, pattern: "^77[0-9]{6}$", type: "tel", title: t("CORE_COMMON_PROFILE_MOBILE_NUMBER_INVALID") }}
                  />
